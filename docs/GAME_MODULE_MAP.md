...
| ------- | ------ | ------------- |
| Shared types, initial save version | `src/game/core/` | `GameState` (incl. `inventoryCostBasis`, `tradeGoldSpent` / `tradeGoldEarned`, `townWarehouses`, `townVisits`, `lastEncounter`), `WarehouseState` (incl. `facilities`, `activeJobs: ProcessingJob[]`), `CaravanState` (incl. `buffs`, `bonusCapacity`), `ProcessingJob`, `TravelEncounter`, IDs, `createInitialState`, `SAVE_VERSION` (currently **v8**) |
...
| Save/load | `src/game/persistence/` | Storage key; save shape migration also in `src/store/gameStore.ts` (Zustand `migrate` + `version`, currently **v8**; migrates caravan buffs / bonus capacity plus warehouse facilities alongside earlier townVisits / encounter / activeJobs defaults) |
