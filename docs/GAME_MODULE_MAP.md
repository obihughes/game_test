...
| ------- | ------ | ------------- |
| Shared types, initial save version | `src/game/core/` | `GameState` (incl. `inventoryCostBasis`, `tradeGoldSpent` / `tradeGoldEarned`, `townWarehouses`, `townVisits`, `lastEncounter`), `WarehouseState` (incl. `activeJobs: ProcessingJob[]`), `ProcessingJob`, `TravelEncounter`, IDs, `createInitialState`, `SAVE_VERSION` (currently **v7**) |
...
| Goods, merchants, buy/sell | `src/game/economy/` | `goods.ts` (**24 goods**: iron, silk, wine, herbs, fresh_fish, salted_fish, smoked_fish, fish_sauce, salt, rope, peat, obsidian_glass, dreaming_moss, crown_amber, fen_spice, coal, metal_tools, grain, timber, pitch, tallow, **polished_amber** (Crownpost craft), **seasoned_spice** (Saltmere craft), **cut_glass** (Stoneholt craft)); `merchants.ts` — town price identity, `townMarketPriceRow`, `bestSellOfferAtTown`, `bestSellPriceAtTown`, `townPrimaryGoods`, `townDemandGoods`, townwide fallback buyer floor; `prices.ts` (`getEffectivePrice`); rules in `buySell.ts`; **`priceHistory.ts`** — `getPriceTrend`, `getPriceTrendDirection`, `trendArrow` (deterministic) |
...
| Town warehouse investments | `src/game/investments/` | `warehouse.ts` — `buildWarehouse`, `upgradeWarehouse`, `depositGoods`, `withdrawGoods`, `processRecipe` (instant), **`startTimedJob`** / **`collectJob`** (timed), `recipesForTown`; `WAREHOUSE_BUILD_COST=200`, `WAREHOUSE_UPGRADE_COST=400`, capacity Lv1=20 / Lv2=50 wt; **12 recipes total**: 6 universal instant + 6 town-specific timed (stoneholt: forge_tools, cut_glass; fenward: brew_pitch; mirecross: press_rope; crownpost: amber_polish; saltmere: spice_blend) |
| Travel events | `src/game/events/` | **`encounters.ts`** — `rollEncounter` (deterministic seed from day+route, ~40% chance per leg); 11 encounter types (road_merchant_offer, bandit_toll, shortcut_found, trader_rumour, broken_wheel, mountain_pass_cold, river_mist, salt_flat_wind, pilgrim_camp, abandoned_cart, crown_inspector); route-specific encounters for mountain/river/salt roads |
| Save/load | `src/game/persistence/` | Storage key; save shape migration also in `src/store/gameStore.ts` (Zustand `migrate` + `version`, currently **v7**; migrates townVisits, lastEncounter, activeJobs defaults) |
| Player-facing text (dialog, flavor) | `src/content/dialog/` | `dialog.ts` keyed strings; **arrival vignettes** keyed as `arrive_{townId}_{season}_{visitBucket}` (first/returning/regular x 4 seasons x 7 towns = 56 vignettes) |
| Location lore + market panel backgrounds | `src/content/locationContent.ts` | `LOCATION_STORIES`, `getLocationPanelBackground`, **`getArrivalVignette`** (season + visit-count based) — all 7 towns; market backdrop gradients still live here while scene art renders in `src/ui/screens/MarketBackdrop.tsx` |
...
| Screens | `src/ui/screens/` | Market (`TownScreen`: single town market buy/sell board, local specialties sorted first, Market tip inline with town title, wanted-good callouts, town-economy backstory, sticky trade summary, **arrival vignette** (seasonal, visit-aware, dismissible), Warehouse tab with instant recipes, **town-specific timed recipes**, **active job progress bars with collect button**, and a town-specific ambient backdrop via `MarketBackdrop.tsx`); map (`MapTravelScreen`); caravan (`CaravanScreen`) |
| React + Zustand wiring | `src/store/gameStore.ts` | `travelTo` (returns `TravelResult | false`; rolls encounter, increments townVisits, stores lastEncounter), `buy` / `sell`, `executeBatch`, `hire` / `dismissHire`, `buildWarehouse`, `upgradeWarehouse`, `depositGoods`, `withdrawGoods`, `processRecipe`, **`startTimedJob`**, **`collectJob`**, **`dismissEncounter`** |
