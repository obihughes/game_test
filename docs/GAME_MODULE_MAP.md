...
| Goods, merchants, buy/sell | `src/game/economy/` | `goods.ts` (21 goods: iron, silk, wine, herbs, **fresh_fish, salted_fish, smoked_fish, fish_sauce**, salt, rope, peat, obsidian_glass, dreaming_moss, crown_amber, fen_spice, **coal, metal_tools, grain, timber, pitch, tallow**; fish labels simplified to **Fresh fish / Salt fish / Smoked fish**); `merchants.ts` (per-town stalls for all 7 towns, `effectivePriceRow`, `bestSellPriceAtTown`, `townPrimaryGoods`, `townDemandGoods`, base-price market identity helpers), `prices.ts` (`getEffectivePrice` ? shared daily variance band ? season multiplier); rules in `buySell.ts`; **`priceHistory.ts`** ? `getPriceTrend`, `getPriceTrendDirection`, `trendArrow` (deterministic, no stored state) |
...
| Screens | `src/ui/screens/` | Market (`TownScreen`: townwide buy/sell board, **local specialties sorted first**, `Market tip` card for **Cheap here / Wanted here**, wanted-good callouts on cargo sales, sticky trade summary, collapsible item details, Warehouse tab); map (`MapTravelScreen`: travel confirm modal); caravan (`CaravanScreen`) |
