...
| ------- | ------ | ------------- |
| Shared types, initial save version | `src/game/core/` | `GameState` (incl. `inventoryCostBasis`, `tradeGoldSpent` / `tradeGoldEarned`, `townWarehouses`, `townVisits`, `lastEncounter`, **`story`**), `WarehouseState` (incl. `facilities`, `activeJobs: ProcessingJob[]`), `CaravanState` (incl. `buffs`, `bonusCapacity`), `ProcessingJob`, `TravelEncounter`, **`StoryState`**, IDs, `createInitialState`, `SAVE_VERSION` (currently **v9**) |
...
| Save/load | `src/game/persistence/` | Storage key; save shape migration also in `src/store/gameStore.ts` (Zustand `migrate` + `version`, currently **v9**; migrates caravan buffs / bonus capacity, warehouse facilities, and **branching story state** alongside earlier townVisits / encounter / activeJobs defaults) |
