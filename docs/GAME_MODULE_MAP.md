...
| Map rendering / colors | `src/ui/map/` (`mapEdgeGeometry.ts`: curved roads + offset `Nd` anchors) + `src/ui/screens/MapTravelScreen.tsx` | Two-column layout: full map left, info panel + destinations sidebar right; travel confirmation modal with cost breakdown + season warning |
