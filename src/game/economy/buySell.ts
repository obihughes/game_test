...
import { PRICES_BY_TOWN } from './prices.ts'
import { spareCapacity } from '../caravan/capacity.ts'

export function buyGood(state: GameState, goodId: GoodId, qty: number): GameResult {
  if (qty <= 0) return { ok: false, reason: 'Invalid amount.' }
  const good = GOODS[goodId]
  if (!good) return { ok: false, reason: 'Unknown good.' }
  const row = PRICES_BY_TOWN[state.location]?.[goodId]
...
export function sellGood(state: GameState, goodId: GoodId, qty: number): GameResult {
  if (qty <= 0) return { ok: false, reason: 'Invalid amount.' }
  const have = state.inventory[goodId] ?? 0
  if (have < qty) {
    return { ok: false, reason: 'Not enough in stock.' }
  }
  const row = PRICES_BY_TOWN[state.location]?.[goodId]
